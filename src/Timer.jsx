...
export function Timer() {
  ...
  return (
    <mesh position={[-3, -11.5, 0]}>
      ...
      >
        <meshBasicMaterial attach="material" color="#ffffff" fillOpacity={100} />
      </circleGeometry>
      { word.map((char, index) => (
        <Text font="/noto.json" key={index} fillOpacity={char.isCollected ? 100 : 0.5} color={0xffffff} fontSize={1} position={[1.2 + index, 0, 1]}>{char.code}</Text>))}
    </mesh>
  );
}
